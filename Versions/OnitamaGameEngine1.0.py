...
start_button.place(x=SIDE*3.85, y=SIDE*3.5)
...
def found_moves(P, card_type):
    ...
    for c in card_moves[card_type]:
        ...
        if (r >= 0 and r <= 4 and c >= 0 and c <= 4):
            if(Board[r][c] == 0):
                NextMove = Move(index, 0, P.row, P.col, r, c)
                FoundMoves.append(NextMove)
            elif (Board[r][c]*index < 0):
                NextMove = Move(index, Board[r][c], P.row, P.col, r, c)
                FoundMoves.append(NextMove)
    return FoundMoves
...
def show_valid_moves(image_index, card_type):
    ...
    if (not is_red_turn):
        turn = 0
    AllMoves = found_moves(color_pawns[turn][image_index], card_type)
